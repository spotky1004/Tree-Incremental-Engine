 ...
 */
/**
 * @typedef {[Resource, CostFunc]} CostChunk
 */

class Cost {
  /** @param {CostChunk[] | CostChunk} option */
  constructor(option) {
    if (option[0] instanceof Resource) option = [option];
    /** @type {[Resource, StagedNumberFunction][]} */
    this.costs = [...option].map(e => [e[0], new StagedNumberFunction(...e[1])]);
    /** @type {Map<string, StagedNumberFunction>} */
    this.costsMap = new Map(this.costs.map(e => [e[0].key, e[1]]));

    // Exception
    let resourceKeys = [...this.costsMap.keys()];
